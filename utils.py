...
class AudioFeatures:
    ...
    def __init__(self,
        ...
        self.melspec_model_predict = lambda x: self.melspec_model.run(output_names=None, input_feed={'input': x})

        # Audio embedding model
        self.embedding_model = ort.InferenceSession(embedding_model_path, sess_options=sessionOptions,
                                                    providers=["CPUExecutionProvider"])
        self.embedding_model_predict = lambda x: self.embedding_model.run(output_names=None, input_feed={'input_1': x})[0].squeeze()
...
# done without mel section
    def _streaming_features(self, x):
        ...
        if self.accumulated_samples + x.shape[0] >= CHUNK:
            # print(f'samples >= CHUNK; {self.accumulated_samples.bit_length()} - {x.shape[0]}')
