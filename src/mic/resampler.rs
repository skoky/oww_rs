...
pub fn resample_audio(data: &[f32], resamplers: &mut Resamplers, channels: usize) -> Result<Vec<f32>, String> {
    ...
    if resamplers.use_upsampler {
        let presampled = resamplers.upsampler.process(&input, 0, None)
            .map_err(|e| format!("upsampler error: {}", e))?;
        let resampled = resamplers.main.process(&presampled, 0, None)
            .map_err(|e| format!("main resampler error: {}", e))?;
        Ok(resampled.take_data())
    } else {
        let resampled = resamplers.main.process(&input, 0, None)
            .map_err(|e| format!("main resampler error: {}", e))?;
        Ok(resampled.take_data())
    }
}
